@extends('layouts.vertical', ['title' => 'Checkout Page Content'])
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                ...
                <div class="card-header d-flex justify-content-between align-items-center bg-white py-3">
                    <h5 class="card-title mb-0 fw-semibold">Checkout Page Content List</h5>
                    <a href="{{ route('checkout-page-contents.create') }}" class="btn btn-primary" id="createButton">
                    ...
                    <table id="datatable" class="table table-bordered dt-responsive table-hover table-responsive nowrap align-middle">
                        ...
                        <tbody>
                            @foreach ($checkoutPages as $page)
                                <tr>
                                    <td>{{ $page->title }}</td>
                                    <td>{{ $page->text_shipping }}</td>
                                    <td>{{ $page->text_payment }}</td>
                                    <td>{{ $page->button_proceed }}</td>
                                    <td>
                                        @if ($page->status)
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('checkout-page-contents.edit', $page->id) }}"
                                                class="btn btn-sm btn-primary">Edit</a>
                                            <form action="{{ route('checkout-page-contents.destroy', $page->id) }}" method="POST"
